...
const MOCK_FILIALEN: Filiale[] = [
  { f_id: 1, name: 'REWE Alexanderplatz', address: 'Alexanderplatz 5, 10178 Berlin' },
  { f_id: 2, name: 'REWE Friedrichstraße', address: 'Friedrichstraße 90, 10117 Berlin' },
  { f_id: 3, name: 'REWE Prenzlauer Berg', address: 'Schönhauser Allee 80, 10439 Berlin' },
  { f_id: 4, name: 'REWE Kreuzberg', address: 'Oranienstraße 25, 10999 Berlin' },
  { f_id: 5, name: 'REWE Charlottenburg', address: 'Wilmersdorfer Str. 46, 10627 Berlin' },
  { f_id: 6, name: 'Penny Neukölln', address: 'Karl-Marx-Straße 112, 12043 Berlin' },
  { f_id: 7, name: 'Penny Wedding', address: 'Müllerstraße 55, 13349 Berlin' },
  { f_id: 8, name: 'Penny Lichtenberg', address: 'Frankfurter Allee 200, 10365 Berlin' },
  { f_id: 9, name: 'Penny Spandau', address: 'Carl-Schurz-Straße 18, 13597 Berlin' },
  { f_id: 10, name: 'Penny Tempelhof', address: 'Tempelhofer Damm 150, 12099 Berlin' },
];
...
// Storage keys
const STORAGE_KEY_HOURS = 'mock_hours_records';
const STORAGE_KEY_NEXT_ID = 'mock_hours_next_id';
...
function generateHistoricalData(): HoursRecord[] {
  ...
  for (let daysAgo = 7; daysAgo >= 1; daysAgo--) {
    ...
    const shuffledMarkets = [...MOCK_FILIALEN].sort(() => Math.random() - 0.5).slice(0, numMarkets);

    for (const market of shuffledMarkets) {
      // 3-6 employees per market per day
      const numEmployees = 3 + Math.floor(Math.random() * 4);
      const shuffledEmployees = [...MOCK_MITARBEITER].sort(() => Math.random() - 0.5).slice(0, numEmployees);
...
export async function submitHours(data: HoursData): Promise<{ success: boolean }> {
  ...
  const newRecord: HoursRecord = {
    ...data,
    id: getNextId(),
    mitarbeiter_name: employee ? `${employee.vorname} ${employee.name}` : undefined,
    filiale_name: market?.name,
  };
